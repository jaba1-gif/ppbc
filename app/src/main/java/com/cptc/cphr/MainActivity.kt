...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(s: Bundle?) {
        ...
        try {
            ...
            Cfg.load(this)
            bindViews()
            // 申请通知权限（Android 13+），前台服务需要
            reqNotificationPermission()
            // 安全启动常驻服务，失败也不崩
            try { startKeepAlive() } catch (e: Exception) {
                Logger.add(this, "前台服务启动失败：${e.message}")
            }
        } catch (e: Exception) {
    ...
    private fun reqNotificationPermission() {
    ...
    override fun onResume() {
        ...
        try {
            ...
            Toast.makeText(this, "刷新异常：${e.message}", Toast.LENGTH_SHORT).show()
        }
    }
    ...
    private fun saveCfg() {
        ...
        Cfg.openWaitMs = (b.etOpenWait.text.toString().toLongOrNull() ?: 4L) * 1000L
